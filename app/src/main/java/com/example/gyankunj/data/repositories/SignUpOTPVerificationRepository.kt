...
class SignUpOTPVerificationRepository(var apiService: gyankunjApiService, var preferenceManager: PreferenceManager) {
    suspend fun verifyOTPCode(phone:String,otp:String):OTPResponse{
        val requestData=VerifyOTPBody(phone,otp)
        return apiService.verifyOTP(requestData)
