...
class ForgotPasswordOTPRepository( val apiService: gyankunjApiService, val preferenceManager: PreferenceManager) {
      suspend fun sendCode(phone:String): OTPResponse {
        val requestData=OTPBody(phone)
         return apiService.otpForgotPassword(requestData)
    }
}
